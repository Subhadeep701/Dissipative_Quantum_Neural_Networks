...
num_experiments = 10
train_ratio = 0.25  # 30% for training, 70% unseen test
...
for exp_idx in range(num_experiments):
    ...
    for fold_idx, (cv_train_idx, cv_val_idx) in enumerate(cv.split(train_data)):
        ...
        for step in range(steps):
            print(f"\n Experiment {exp_idx+1}/{num_experiments}| CV Fold {fold_idx+1}/{k_folds}| Step {step+1}/{steps}")

            # --- Method B ---
            entropy_plot_B, unitaries_B = safe_qnnTraining_methodB(qnnArch, unitaries_B, cv_train, lda=3, ep=0.1, rounds=training_intervals)
            train_entropy_B.append(evaluate_qnn(qnnArch, unitaries_B, cv_train))
            cv_entropy_B.append(evaluate_qnn(qnnArch, unitaries_B, cv_val))
            train_fid_B.append(evaluate_fidelity(qnnArch, unitaries_B, cv_train))
            cv_fid_B.append(evaluate_fidelity(qnnArch, unitaries_B, cv_val))
            # Evaluate on unseen test step by step
            test_entropy_B.append(evaluate_qnn(qnnArch, unitaries_B, test_data))
            test_fid_B.append(evaluate_fidelity(qnnArch, unitaries_B, test_data))

            # --- Method A ---
            entropy_plot_A, unitaries_A = safe_qnnTraining_methodA(qnnArch, unitaries_A, cv_train, lda=3, ep=0.1, rounds=training_intervals)
            train_entropy_A.append(evaluate_qnn(qnnArch, unitaries_A, cv_train))
            cv_entropy_A.append(evaluate_qnn(qnnArch, unitaries_A, cv_val))
            train_fid_A.append(evaluate_fidelity(qnnArch, unitaries_A, cv_train))
            cv_fid_A.append(evaluate_fidelity(qnnArch, unitaries_A, cv_val))
            # Evaluate on unseen test step by step
            test_entropy_A.append(evaluate_qnn(qnnArch, unitaries_A, test_data))
            test_fid_A.append(evaluate_fidelity(qnnArch, unitaries_A, test_data))
